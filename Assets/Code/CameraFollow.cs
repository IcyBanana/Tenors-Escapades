...
public class CameraFollow : MonoBehaviour
{
    ...
    void Zoom () { // Zooms the camera out by moving it back on the Z axis based on player speed.
        ...
        //float zoom = (maxZoomZ - originalZ) * currentZoomPercent + originalZ; // Apply percentage to get the final zoom product.
        //float lerpedZoom = Mathf.Lerp(transform.position.z, zoom, zoomLerpSpeed * Time.fixedDeltaTime); // Lerp to desired zoom product. (Smooths out camera zooms)
        //myCamera.fieldOfView = Mathf.Lerp(myCamera.fieldOfView, zoomFOVBounds.x + (zoomFOVBounds.y - zoomFOVBounds.x) * currentZoomPercent, zoomLerpSpeed * Time.fixedDeltaTime);
