...
public class PlayerController : MonoBehaviour
{
    ...
    private bool isInsideRopeTrigger = false;
    ...
    public void SetInsideRopeTrigger (Collider2D ropeEdgeCollider, bool isInside) {
        isInsideRopeTrigger = isInside;
        lastRopeCol = ropeEdgeCollider;
    }
    ...
    void RecoverCrash () {
        ...
        if(diedInChasm) {
            float xPos = gameManager.currentChunk.GetStartEnd().x + 2f;
            float yPos = gameManager.currentChunk.GetHeightOnSpline(xPos - 2f);
    ...
    void RopeGrind () {    
        if(isOnRope) {  
            ...
            if(ropeGrindAudioSource.isPlaying) 
                ropeGrindAudioSource.Stop();
            if(lastRopeCol) {
                if(AttemptRopeGrind(lastRopeCol)) {
                    lastRopeCol.enabled = true;
                }
                else {
                    isOnRope = false;
                    isGrinding = false;
                    lastRopeCol.enabled = false;
                }
            }
        }
    }
    ...
    bool AttemptRopeGrind (Collider2D col) {
        ...
        float dotV = Vector2.Dot(playerRigidbody.velocity.normalized, normal);
        float errorMargin = 0.5f;

        if(dot >= 0.5f && dotV <= errorMargin)
            return true;
        else
            return false;
    }
    ...
    */
    ...
    void OnTriggerEnter2D(Collider2D col) {
        ...
        else if(col.gameObject.layer == 11 && !col.isTrigger) {
            lastRopeCol = col;
        }
    }
    ...
    void OnCollisionEnter2D (Collision2D col) {
        ...
        else if(col.gameObject.layer == 11 && !col.collider.isTrigger) {
            isOnRope = true;
            hasJumped = true;
            isGrinding = true;
    ...
    void OnCollisionExit2D (Collision2D col) {
        if(col.gameObject.layer == 11 && !col.collider.isTrigger) {
            isOnRope = false;
            isGrinding = false;
        }
    }
